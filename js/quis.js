        ...
        <section id="view-quiz" class="view active">
            <div class="quiz-container" style="max-width:600px;margin:0 auto;padding:24px 16px;">
                ...
                <div class="quiz-card" style="background:var(--surface);border:3px solid var(--border);border-radius:24px;padding:24px;box-shadow:var(--shadow-soft);margin-bottom:20px;">
                    <div id="quiz-visual" style="text-align:center;margin-bottom:20px;" class="hidden"></div>
...
function renderVisual(visualEl, visual) {
    visualEl.innerHTML = "";
    visualEl.classList.add("hidden");

    if (!visual) return;

    const value = String(visual).trim();
    ...
    if (isImage) {
        ...
        visualEl.style.fontSize = "64px";
        visualEl.style.lineHeight = "1.2";
    }
...
function renderQuestion() {
    ...
    (q.options || []).forEach((opt, idx) => {
        ...
        btn.style.cssText =
            "display:flex;align-items:center;gap:14px;background:#F8FAFC;border:2px solid var(--border);border-radius:16px;padding:16px;text-align:left;font-size:16px;font-weight:600;cursor:pointer;transition:all .2s;";
        ...
        label.style.cssText =
            "display:flex;align-items:center;justify-content:center;min-width:32px;height:32px;border-radius:50%;background:#E2E8F0;color:#1E293B;font-weight:800;flex-shrink:0;";
...
async function handleAnswer(optionIndex, selectedOpt, questionObj) {
    ...
    feedbackEl.style.cssText =
        `margin-top:16px;padding:16px;border-radius:16px;background:${isCorrect ? "#F0FDF4" : "#FEF2F2"};color:${isCorrect ? "#166534" : "#B91C1C"};border:2px solid ${isCorrect ? "#86EFAC" : "#FCA5A5"};`;
...
function renderResultScreen(resultData) {
    ...
    const correct =
        resultData.correct_count || quizState.correctCount;
    const wrong =
        resultData.wrong_count || quizState.wrongCount;
            ...
            <div class="result-container" style="max-width:480px;margin:40px auto;padding:20px;text-align:center;">
                <div class="result-card" style="background:var(--surface);border:3px solid var(--border);border-radius:28px;padding:40px 24px;box-shadow:var(--shadow-soft);">
                    ...
                    <div style="background:#EFF6FF;border:2px dashed #93C5FD;border-radius:20px;padding:20px;margin-bottom:24px;">
                        ...
                        <h1 style="font-size:48px;color:var(--blue);margin:0;">
                            ${finalScore}
                        </h1>
                    </div>
