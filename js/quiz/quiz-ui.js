...
export function renderQuestion(state, callbacks) {
        ...
        <div class="quiz-card" style="background:var(--surface);border:3px solid var(--border);border-radius:24px;padding:24px;box-shadow:var(--shadow-soft);">
            ${imageSource ? `
                <div style="text-align:center;margin-bottom:18px;">
                    ${questionObj.image ? `
                        <img src="${escapeAttribute(imageSource)}" alt="" style="max-width:180px;max-height:140px;object-fit:contain;">
                    ` : `
                        <div style="font-size:72px;line-height:1;">${escapeHtml(imageSource)}</div>
                    `}
                </div>
            ` : ""}
    ...
    document.querySelectorAll(".option-card").forEach((button, index) => {
        button.addEventListener("click", () => {
            callbacks.onAnswer(index, questionObj.options[index], questionObj);
        });
    });

    document.getElementById("btn-next-question").addEventListener("click", callbacks.onNext);
}
...
function renderOptions(questionObj) {
    ...
    return (questionObj.options || []).map((option, index) => `
        <button class="option-card" type="button" data-option-index="${index}" style="width:100%;display:flex;align-items:center;gap:12px;text-align:left;background:var(--surface);border:2px solid var(--border);border-radius:16px;padding:14px;margin-top:12px;font-size:16px;font-weight:600;cursor:pointer;">
            <span style="width:34px;height:34px;min-width:34px;border-radius:50%;background:var(--border);display:flex;align-items:center;justify-content:center;font-weight:700;">
...
export function showAnswerFeedback(questionObj, optionIndex, isCorrect) {
    ...
    feedback.innerHTML = `
        ...
        <span>${escapeHtml(questionObj.explanation || "Tetap semangat mencoba ya!")}</span>
    `;
