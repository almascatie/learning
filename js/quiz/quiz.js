...
import { saveQuizProgress, getQuizProgress, clearQuizProgress } from "./quiz-progress.js";
import { getActiveSession, getActiveSessionToken, saveStudentAnswer, completeStudentAttempt, startStudentAttempt } from "./quiz-sync.js";
import { renderQuizContainer, renderQuestion, showAnswerFeedback } from "./quiz-ui.js";
...
export async function openQuizPackages(grade) {
    ...
    if (!packages.length) {
        packageList.innerHTML = `<div class="loading">Belum ada paket kuis untuk Kelas ${studentGrade}.</div>`;
        return;
    }
    ...
    if (backButton) {
        backButton.onclick = () => showView("view-home");
    }
}
...
async function loadQuizPackage(quizPackage) {
    try {
        ...
        if (!packageData) throw new Error("File paket tidak memiliki export default.");
        if (!Array.isArray(packageData.questions)) throw new Error("Paket tidak memiliki array questions.");
...
export async function startQuiz(packageId, questionsArray) {
    ...
    try {
        const progress = await getQuizProgress(session, packageId, questionsArray.length);

        if (progress) {
            quizState.attemptId = progress.attempt_id;
            quizState.currentIndex = Math.min(progress.currentIndex || 0, questionsArray.length - 1);
            quizState.correctCount = progress.correctCount || 0;
            quizState.wrongCount = progress.wrongCount || 0;
        } else {
            quizState.attemptId = await startStudentAttempt(sessionToken, packageId, questionsArray.length);
            await saveQuizProgress(session, quizState);
        }

        renderQuizContainer(quizState, {
            onQuit: quitQuiz,
...
async function handleAnswer(optionIndex, selectedOpt, questionObj) {
    ...
    showAnswerFeedback(questionObj, optionIndex, isCorrect);
    ...
    try {
        ...
        if (session) await saveQuizProgress(session, quizState);
    } catch (err) {
...
async function finishQuiz() {
    ...
    try {
        const resultData = await completeStudentAttempt(
            sessionToken,
            quizState.attemptId
        );

        const session = await getActiveSession();
        if (session) await clearQuizProgress(session, quizState.packageId);

        renderQuizResult(resultData || {}, quizState, {
            onBack: () => openQuizPackages(session?.grade)
        });
    } catch (err) {
...
async function quitQuiz() {
    ...
    if (!confirm("Keluar dari kuis? Progress saat ini akan disimpan.")) return;

    showView("view-quiz-packages");

    if (session) {
        openQuizPackages(session.grade);
    }
}
